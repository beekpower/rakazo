...
export function MessageHoverUsage({ children }: { children: ReactNode }) {
  return (
    <span
      ...
      className="pointer-events-none absolute top-full start-0 z-10 mt-1 whitespace-nowrap text-xs tabular-nums text-muted-foreground opacity-0 transition-opacity group-hover/message:opacity-100 group-focus-within/message:opacity-100 group-has-[[aria-expanded=true]]/message:opacity-100"
    >
      {children}
    </span>
  );
